...
const validateUserEmail = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (user?.googleId) {
      res.status(400).json({
        message: "This is a google account, reset your google password!",
      });
    }
...
const forgotPasswordOtpGeneration = asyncHandler(async (req, res) => {
  ...
  await user
    ...
    .catch((err) => {
      ...
      res
        .status(400)
        .json({ proceed: false, message: "Something went wrong, try again" });
    });
});
  ...
  try {
    ...
    if (!user) {
      ...
      if (user?.otpExpiry > Date.now()) {
        ...
        res
          .status(400)
          .json({ proceed: false, message: "OTP Expired, generate new OTP" });
      }
    }
  } catch (error) {
    console.log(error);
